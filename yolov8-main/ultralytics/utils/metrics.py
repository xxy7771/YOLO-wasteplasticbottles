...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, eps=1e-7, scale=0.0):
    """
...
#                 with open(f'b/{names[i]}_pose.csv', 'w+') as f:
...
#         on_plot(save_dir)
def plot_pr_curve(px, py, ap, save_dir=Path('pr_curve.png'), names=(), on_plot=None):
    """Plots a precision-recall curve and saves per-class and all-classes data."""
    fig, ax = plt.subplots(1, 1, figsize=(9, 6), tight_layout=True)
    py = np.stack(py, axis=1)

    # Create directory to save data files if not exists
    save_dir.parent.mkdir(parents=True, exist_ok=True)

    # Plot per-class PR curves if names are provided and less than 21
    if 0 < len(names) < 21:
        for i, y in enumerate(py.T):
            ax.plot(px, y, linewidth=1, label=f'{names[i]} {ap[i, 0]:.3f}')  # plot(recall, precision)
            # Save per-class data to CSV
            filename = f'b/{names[i]}_{"pose" if "Pose" in str(save_dir) else "Box"}.csv'
            with open(filename, 'w') as f:
                f.write('Recall,Precision\n')
                for px_v, y_v in zip(px, y):
                    f.write(f'{px_v},{y_v}\n')

    # Plot all-classes mean PR curve
    mean_py = py.mean(1)
    ax.plot(px, mean_py, linewidth=3, color='blue', label=f'all classes {ap[:, 0].mean():.3f} mAP@0.5')

    # Save all-classes data to CSV
    with open('all_classes.csv', 'w') as f:
        f.write('Recall,Precision\n')
        for px_v, mean_y_v in zip(px, mean_py):
